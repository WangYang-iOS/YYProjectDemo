...
#define kBMSafeH ([UIScreen mainScreen].bounds.size.height >= 812.0 ? 34 : 0)

@interface YYPhotoBrowser ()<UIScrollViewDelegate, UIViewControllerTransitioningDelegate, CAAnimationDelegate, YYPhotoTopViewDelegate, YYPhotoBottomViewDelegate> {
    CGPoint _startLocation;
}

@property (nonatomic, strong) UIScrollView *scrollView;
@property (nonatomic, strong) YYPhotoTopView *topView;
@property (strong, nonatomic) YYPhotoBottomView *bottomView;
...
+ (instancetype)browserWithPhotoItems:(NSArray<YYPhotoItem *> *)photoItems selectedIndex:(NSUInteger)selectedIndex delegate:(id<YYPhotoBrowserDelegate>)delegate {
    YYPhotoBrowser *browser = [[YYPhotoBrowser alloc] initWithPhotoItems:photoItems selectedIndex:selectedIndex delegate:delegate];
    return browser;
}
- (instancetype)initWithPhotoItems:(NSArray<YYPhotoItem *> *)photoItems selectedIndex:(NSUInteger)selectedIndex {
    YYPhotoBrowser *browser = [[YYPhotoBrowser alloc] initWithPhotoItems:photoItems selectedIndex:selectedIndex delegate:nil];
    return browser;
}
...
- (void)viewDidLoad {
    ...
    _bottomView = [[YYPhotoBottomView alloc] initWithFrame: CGRectMake(0, SCREENH - kBMSafeH - 49, SCREENW, 49) delegate:self];
...
- (void)topView:(UIView *)topView didSelectedItem:(UIButton *)button {
    ...
    for (int i = 0; i < _photoItems.count; i++) {
        YYPhotoItem *item = _photoItems[i];
        if(item.asset.selected){
    ...
    if (![array containsObject:item]) {
        if (array.count == self.maxCount) {
            if (self.maxCount == 1) {
                YYPhotoItem *item = array.firstObject;
                item.asset.selected = NO;
            }else {
                NSString *hud = [NSString stringWithFormat:@"最多可选%ld张",(long)self.maxCount];
                HUD(hud)
                return;
            }
        }
...
#pragma mark
#pragma mark -- YYPhotoBottomViewDelegate
...
- (void)didClickSureAtBottomView:(UIView *)bottomView {
    ...
    if(array.count == 0) {
        YYPhotoItem *item = [_photoItems objectAtIndex:_currentPage];
        [imgs addObject:item.asset.coverImage];
    }else {
        for (int i = 0; i < array.count; i++) {
            YYPhotoItem *item = array[i];
            [imgs addObject:item.asset.coverImage];
        }
    }
    [self showDismissalAnimation];
    if ([self.delegate respondsToSelector:@selector(photoBrowser:didClickSureButton:)]) {
        [self.delegate photoBrowser:self didClickSureButton:imgs];
    }
}
